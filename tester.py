...
class Tester:
	...
	def __init__(self, options):
		...
		self.options = {
			'browser':
				options['browser']
				if 'browser' in options and options['browser'] else False,
			'html':
				options['html'] if 'html' in options else False
		}
	...
	def run(self, git_version):
		...
		accepts_license_files = git_version >= 'v1.2' or not re.match(
			r'^v\d+.\d+$', git_version
		)
		...
		with tempfile.TemporaryDirectory(dir = './tests') as tmpfolder:
			...
			if git_version != 'current':
				...
				subprocess.run(
					['git', 'clone', self.git_repo, tempgit],
					capture_output = True,
					check = True
				)
				subprocess.run(
			...
			for f in self.files:
				...
				if os.path.isdir(f) and not accepts_license_files:
					for g in os.scandir(f):
						if re.search(r'\.license$', g.name):
							os.rename(
								g,
								os.path.join(tmpfolder, os.path.basename(g))
							)
				...
				subprocess.run(
					['mkdir', '-p',
						os.path.join(self.savedir, git_version)],
					check = True
				)
				jsonfile = os.path.join(
					self.savedir, git_version,
					os.path.basename(f) + '.json'
				)
				subprocess.run(
					args = [kumiko_bin, '-i', f, '-o', jsonfile, '--progress'],
					check = True
				)
	...
	def compare_all(self):
		for i in range(len(self.git_versions) - 1):
			...
			print(
				'\n\n########## Comparing kumiko results between versions', v1,
				'and', v2, '##########'
			)
			...
			for file_or_dir in self.files:
				...
				if len(files_diff) > 20:
					print(
						'The maximum number of differences in files (20) has been reached, stopping'
					)
					break
				...
				with open(
					os.path.join(
						self.savedir, v1,
						os.path.basename(file_or_dir) + '.json'
					),
					encoding = "utf8"
				) as fh:
					json1 = json.load(fh)
				...
				files_diff.update(
					Debug.get_files_diff(file_or_dir, json1, json2)
				)
			...
			html_diff_file = os.path.join(
				self.savedir, 'diff-' + v1 + '-' + v2 + '.html'
			)
			with open(html_diff_file, 'w', encoding = "utf8") as diff_file:
				diff_file.write(
					HTML.header('Comparing Kumiko results', '../../')
				)
			...
			if self.options['browser']:
				subprocess.run(
					[self.options['browser'], html_diff_file], check = True
				)
...
parser.add_argument(
	...
	nargs = '?',
	help =
	"Just 'run' (compute information about files), 'compare' two versions of the code, or both",
...
parser.add_argument(
	...
	nargs = 1,
	help =
	'Opens given browser to view the differences when ready (implies --html)',
	choices = ['firefox', 'konqueror', 'chromium']
)
parser.add_argument(
	'--html',
	action = 'store_true',
	help = 'Generates an HTML file showing the differences between code versions'
)
